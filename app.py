...
app.config['ALLOWED_EXTENSIONS'] = {'txt', 'pdf', 'png', 'jpg', 'jpeg', 'gif', 'mp3', 'wav', 'avi', 'mp4', 'mov', 'm4a', 'webm', 'aac', 'flac', 'mid'}
...
#app.config['static_url'] = 'http://127.0.0.1:541/static/'
#app.config['jscss_url'] = 'http://127.0.0.1:541/static/'
...
def handle_exception(e):
    app.logger.warning(f'错误 {request.method} {request.url} {e}')
    return render_template('error.html', continue_url='/', h1_text=f"出现了一个错误")
...
def index():
    messages = MessageManager.get_hot_messages(like_list=session.get('likes', []), dislike_list=session.get('dislikes', []))
    return render_template('index.html', static_url=app.config['static_url'], jscss_url=app.config['jscss_url'], messages=messages)
...
@app.route('/api/get_message_details/<message_id>')
...
def wall_submit():
    ...
    try:
        MessageManager.post_message(text=text, files=valid_files)
...
def wall_main():
    ...
    messages = MessageManager.get_messages(like_list=session.get('likes', []), dislike_list=session.get('dislikes', []), sort=sort, word=word, filter=filter_critical)
    if word:
        return render_template('wall.html', static_url=app.config['static_url'], jscss_url=app.config['jscss_url'], messages=messages[0:MESSAGE_PAGE_SIZE], defaultSort=sort, search_word=word, defaultFilter=filter_critical, search_result=f"找到以下有关'{word}'的{len(messages)}条内容")
    return render_template('wall.html', static_url=app.config['static_url'], jscss_url=app.config['jscss_url'], messages=messages[0:MESSAGE_PAGE_SIZE], defaultSort=sort, defaultFilter=filter_critical)
